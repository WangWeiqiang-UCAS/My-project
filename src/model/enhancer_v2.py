...
class GeneralPurposeEnhancer_V2(nn.Module):
    ...
    def __init__(
        ...
        self.grm = VQFeatureRefinementModule_V2(channels=grm_shared_channels, prompt_dim=prompt_dim, num_heads=4)
    ...
    def _print_info(self):
        ...
        proj_params = sum(p.numel() for p in self.grm_in_projs.parameters()) + sum(
            p.numel() for p in self.grm_out_projs.parameters()
        )
        ...
        print("\n💡 使用提示:")
        print("  1. 输入: image (B,3,H,W) + feature_pyramid [List of Tensors]")
        print("  2. 输出: enhanced_features (相同shape) + info (Dict)")
        print("  3. 即插即用: 可接入任意骨干网络 (YOLO/ResNet/ViT/...)")
        print("  4. 可控编辑: 通过操纵 prompt_indices 实现精确控制")
        print(f"{'=' * 80}\n")
    ...
    def forward(
        ...
        use_vq: bool = True,
        custom_prompt_indices: torch.Tensor | None = None,
        ...
        Args:
            ...
            custom_prompt_indices: 自定义的提示码索引 (B, N) 用于可控编辑，如果提供则使用它而不是自动分析
        ...
        Returns:
            ...
            示例: >>> enhancer = GeneralPurposeEnhancer_V2([64, 128, 256]) >>> image = torch.randn(4, 3, 256, 256) >>>
                features = [torch.randn(4, 64, 32, 32), torch.randn(4, 128, 16, 16), torch.randn(4, 256, 8, 8)] >>>
                enhanced, info = enhancer(image, features) >>> print([f.shape for f in enhanced]) [(4, 64, 32, 32), (4,
                128, 16, 16), (4, 256, 8, 8)]
        """
        # ========================================
        # 阶段 1: 场景分析与提示生成
    ...
    def manipulate_prompt(
        ...
        Returns:
            new_indices: 修改后的索引
            示例: >>> # 将左上角区域强制使用"强去雨"策略 >>> mask = create_topleft_mask(batch_size, num_patches) >>> new_indices =
                enhancer.manipulate_prompt( ... original_indices, ... operation="add", ... target_code=100, # 假设 100
                代表强去雨 ... positions=mask, ... )
        """
        return self.spm.manipulate_prompt(
            original_indices, operation=operation, target_code=target_code, source_code=source_code, positions=positions
        )
...
def build_enhancer_for_yolo(
    img_size: int = 640, pretrained_path: str | None = None, freeze: bool = False
    ...
    if pretrained_path:
        ...
        if "enhancer_state_dict" in checkpoint:
            ...
            print("⚠️  Checkpoint 中未找到 'enhancer_state_dict'\n")
...
def build_enhancer_for_resnet(
    img_size: int = 224, pretrained_path: str | None = None, freeze: bool = False
) -> GeneralPurposeEnhancer_V2:
    """为 ResNet 系列构建增强器."""
    enhancer = GeneralPurposeEnhancer_V2(
