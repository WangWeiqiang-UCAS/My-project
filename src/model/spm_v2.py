    ...
    def compute_statistics(self, image: torch.Tensor) -> torch.Tensor:
        ...
        return torch.cat(
            [mean, std, min_val, max_val,
             skewness.unsqueeze(-1), kurtosis.unsqueeze(-1), dark_ratio.unsqueeze(-1)],
            dim=-1
        )
...
class PromptSequenceVQ(nn.Module):
    ...
    def forward(self, z: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, Dict]:
        ...
        if self.training:
            ...
            # EMA 更新
            with torch.no_grad():
                self.code_usage.add_(
                    torch.bincount(encoding_indices, minlength=self.num_embeddings)
                )
                ...
                if self.batch_count % 100 == 0:
                    ...
                    if len(dead_codes) > 0 and z_flat.size(0) >= len(dead_codes):
                        alive_indices = torch.randperm(
                            z_flat.size(0), device=z.device
                        )[:len(dead_codes)]
...
class ScenePromptModule_V2(nn.Module):
    ...
    def __init__(
        ...
        self.pos_embed = nn.Parameter(
            torch.randn(1, self.num_patches, prompt_dim) * 0.02
        )
    ...
    def _interpolated_pos_embed(self, device: torch.device, dtype: torch.dtype, target_h: int, target_w: int) -> torch.Tensor:
        """
    ...
    def forward(self, image: torch.Tensor, use_vq: bool = True) -> Dict:
        ...
        if cur_num_patches != self.num_patches:
            ...
            if H_img % self.patch_size != 0 or W_img % self.patch_size != 0:
                ...
                print(f"⚠️ 输入图像大小 ({H_img}x{W_img}) 不能被 patch_size ({self.patch_size}) 整除。"
                      f" 将按整除下取整的网格 ({cur_h}x{cur_w}) 处理。")
            pos = self._interpolated_pos_embed(device=patch_seq.device, dtype=patch_seq.dtype, target_h=cur_h, target_w=cur_w)
        else:
